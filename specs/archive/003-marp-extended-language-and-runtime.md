...
fragment playback, and presenter notes, while preserving existing Kami decks and
command IDs.
...
- [x] Generic Marp Extended markers compile to stable namespaced markup and official Marpit directives, while all existing Kami markers, classes, source files, and command IDs remain compatible.
...
- [x] Preview exposes Marpit presenter comments for the active slide in a toggleable, accessible notes panel without changing the legacy Kami `%%marp-note%%` visual block.
...
- [x] User and developer documentation accurately describes the supported Marpit baseline, Extended syntax, legacy aliases, runtime controls, export parity, and HTML policy.
...
- A generic Extended syntax/compiler contract, compatibility aliases, structural CSS, templates, and conformance tests.
...
- Migration-safe HTML handling and durable documentation.

Not in scope:

- Replacing standard Marpit directives, image syntax, theme CSS, or slide separators with redundant Extended syntax.
- Rewriting existing notes on disk or removing legacy Kami markers/classes/commands.
...
| 2026-08-09 | Existing `%%marp-*%%` source forms and `insert-kami-*` command IDs remain supported; generic forms add namespaced structural classes alongside legacy classes. | Existing decks and user keybindings are compatibility surfaces. | WS-01 |
| 2026-08-09 | `%%marp-note%%` remains a visual Kami callout; presenter notes continue to use ordinary Marpit comments. | Avoids silently changing rendered content into hidden metadata. | WS-01, WS-03 |
...
| 2026-08-09 | Preview retains trusted-author `html: true` for generated Mermaid SVG and legacy raw HTML, while the iframe is sandboxed without author script execution if the runtime smoke passes. | Core's safe allowlist cannot reliably cover Mermaid SVG; sandboxing limits preview risk without breaking export compatibility. | WS-02, WS-03, WS-04 |
...
| --- | --- | --- | --- | --- | --- |
| WS-01 | Generic Extended compiler, legacy Kami aliases, structural CSS, templates, and conformance fixtures | Amp | Done | None | Focused DSL, Markdown pipeline, theme, and template tests |
...
- Manual preview smoke covers fragment backward/forward/reset, active-slide changes, notes visibility, rerender state, and a legacy Kami deck.
...
- Changed: Added the generic Extended aliases and namespaced structural contract, retained legacy Kami source/classes/commands, hardened nested/fenced marker compilation, added fragments and presenter notes, and routed preview plus managed export through fresh instances of the shared Core 5 engine contract.
...
- Review evidence: Independent review findings for recursive markers, legacy callout precedence, Windows/concurrent engine materialization, stale iframe commits, and shared preview engine reuse were fixed. Regression tests cover each concurrency and compatibility boundary.
- Verification: `npm run sync:themes`, `npm run check:specs`, `npm run typecheck`, `npm run lint`, `npm test -- --runInBand` (155 passed, 1 skipped), `npm run build`, `RUN_REAL_MARP_EXPORTS=1 npm run test -- --runInBand tests/unit/marpExportReal.test.ts`, `npm run scan:release`, and `git diff --check` passed. Lint retained two pre-existing warnings and no errors.
...
adds a backward-compatible, theme-independent authoring layer for higher-level
...
artifacts carry a verified standalone engine with an embedded fallback. Legacy
Kami decks and command IDs remain supported.
